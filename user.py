...
def get_user_list(secure=True):
    users=json.load(open(main.rootfolder+"users.json"))["users"]
    if secure:
        for user in users:
            user.pop("password")
    return str(users)
...
def create_user(name,displayname="",password=""):
    with open(main.rootfolder+"users.json","r") as user_file:
    ...
    with open(main.rootfolder+"users.json","w") as user_file:
